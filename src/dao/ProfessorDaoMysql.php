...
class ProfesorDaoMySql implements ProfessorDao
{
    ...
    public function findByName($name)
    {
        ...
        $array = [];
        $sql = $this->pdo->prepare("SELECT * FROM tb_user WHERE nome like :name");
        ...
        if ($sql->rowCount() > 0) {
            ...
            foreach ($data as $item) {
                // Verifica se existe um professor vinculado a este usuário
                $professor = $this->findByUserId($item['id']);

                if ($professor) {
                  
                    $rating = $this->handleRating($professor->getRating(), $professor->getQuantidadeAulasAplicadas());
                    $professor->setRating($rating);

                    $array[] = $professor; // Adiciona ao array final
                }
            }
        }
